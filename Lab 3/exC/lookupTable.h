...
template<class K, class T> class LookupTable; // Forward defining to stop approx 40 error messages.
...
template<class K, class T>
class LT_Node {
  friend class LookupTable<K,T>;
private:
  Pair<K,T> pairM;
  LT_Node<K,T> *nextM;

  // This ctor should be convenient in insert and copy operations.
  LT_Node(const Pair<K,T>& pairA, LT_Node<K,T> *nextA);
};
...
//    LT_Node* cursor;
  ...
  public:
    Iterator():LT(0){}
    Iterator(LookupTable<K,T>& x): LT(&x){}
    const T& operator *();
    const T& operator ++();
  ...
  LookupTable();
  LookupTable(const LookupTable<K,T>& source);
  LookupTable& operator =(const LookupTable<K,T>& rhs);
  ~LookupTable();

  LookupTable<K,T>& begin();
  ...
  void insert(const Pair<K,T>& pariA);
  // PROMISES:
  ...
  template<class K2, class T2> //fixes shadowing error
  friend ostream& operator << (ostream& os, const LookupTable<K,T>& lt);
 ...
 private:
  ...
  void copy(const LookupTable<K,T>& source);
...
template<class K, class T>
LookupTable<K,T>& LookupTable<K,T>::begin(){
  cursorM = headM;
  return *this;
}

template<class K, class T>
LT_Node<K,T>::LT_Node(const Pair<K,T>& pairA, LT_Node<K,T> *nextA)
  : pairM(pairA), nextM(nextA)
{
}

template<class K, class T>
LookupTable<K,T>::LookupTable()
  : sizeM(0), headM(0), cursorM(0)
{
}

template<class K, class T>
LookupTable<K,T>::LookupTable(const LookupTable<K,T>& source)
{
  copy(source);
}

template<class K, class T>
LookupTable<K,T>& LookupTable<K,T>::operator =(const LookupTable<K,T>& rhs)
{
  if (this != &rhs) {
    destroy();
    copy(rhs);
...
template<class K, class T>
int LookupTable<K,T>::size() const
{
  return sizeM;
}

template<class K, class T>
int LookupTable<K,T>::cursor_ok() const
{
  return cursorM != 0;
}

template<class K, class T>
const K& LookupTable<K,T>::cursor_key() const
{
  assert(cursor_ok());
...
template<class K, class T>
const T& LookupTable<K,T>::cursor_datum() const
{
  assert(cursor_ok());
...
template<class K, class T>
void LookupTable<K,T>::insert(const Pair<K,T>& pairA)
{
  // Add new node at head?
  if (headM == 0 || pairA.key < headM->pairM.key) {
    headM = new LT_Node<K,T>(pairA, headM);
    sizeM++;
  }
  ...
  // Have to search ...
    ...
    LT_Node<K,T>* before= headM;
    LT_Node<K,T>* after=headM->nextM;
	...
	after->pairM.datum = pairA.datum;
      ...
      {
	before->nextM = new LT_Node<K,T>(pairA, before->nextM);
	sizeM++;
      }
  }
}
...
template<class K, class T>
void LookupTable<K,T>::remove(const K& keyA)
{

  if (headM == 0 || keyA < headM->pairM.key)
    return;

  LT_Node<K,T>* doomed_node = 0;
  if (keyA == headM->pairM.key) {
    ...
    LT_Node<K,T>* before = headM;
    LT_Node<K,T>* maybe_doomed = headM->nextM;
...
template<class K, class T>
void LookupTable<K,T>::find(const K& keyA)
{
  LT_Node<K,T> *ptr=headM;
...
template<class K, class T>
void LookupTable<K,T>::go_to_first()
{
  cursorM = headM;
}

template<class K, class T>
void LookupTable<K,T>::step_fwd()
{
  assert(cursor_ok());
...
template<class K, class T>
void LookupTable<K,T>::make_empty()
{
  destroy();
  sizeM = 0;
  cursorM = 0;
...
template<class K, class T>
void LookupTable<K,T>::copy(const LookupTable<K,T>& source)
{
  ...
  for(LT_Node<K,T> *p = source.headM; p != 0; p=p->nextM)
    {
...
template<class K, class T>
ostream& operator <<(ostream& os, const LookupTable<K,T>& lt)
{
  if (lt.cursor_ok())
...
template<class K, class T>
const T& LookupTable<K,T>::Iterator::operator *()
{
  assert(LT ->cursor_ok());
...
template<class K, class T>
const T& LookupTable<K,T>::Iterator::operator ++(int)
{
  assert(LT->cursor_ok());
...
template<class K, class T>
int LookupTable<K,T>::Iterator::operator!()
{
  return (LT->cursor_ok());
}
