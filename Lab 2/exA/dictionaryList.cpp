...
const Datum& Node::get_datum() const{
    return datumM;
}
...
ostream& operator <<(ostream& out, const DictionaryList& list){
    const Node* temp_cursor = list.headM;

    for(int i = 0; i < list.size(); i++){
        cout << temp_cursor -> get_key() << " " << temp_cursor -> get_datum() << endl;
        temp_cursor = temp_cursor -> get_next();
    }
